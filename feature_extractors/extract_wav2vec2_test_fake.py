# runner_wav2vec2_test.py — TEST feature extractor (MMS / EDGE / ALL), resume-safe
...
# ======= EDIT THESE =======
ROOT     = Path(r"G:\My Drive\hindi_dfake")  # your real root
...
for d in (FEAT_DIR, META_DIR, JOBS_DIR, HF_CACHE): d.mkdir(parents=True, exist_ok=True)
...
else: torch.set_float32_matmul_precision("high")
...
if not DRY_RUN and rows_buf:
    append_manifest_rows(rows_buf); rows_buf.clear()
    todo.to_csv(todo_csv, index=False)
print("[ok] Wav2Vec2 test features done.")
