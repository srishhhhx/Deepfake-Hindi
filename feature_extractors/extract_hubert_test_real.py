# ===== HuBERT-Base — FULL-SCAN runner (FLEURS-only; resumable; strong-only; no CSV edits) =====
from pathlib import Path
import os, csv, time, gc, sys, signal, hashlib
import numpy as np, pandas as pd
from tqdm import tqdm

# ---------- CONFIG ----------
ROOT        = Path(r"G:\My Drive\hindi_dfake")   # adjust to "/content/drive/MyDrive/hindi_dfake" on Colab
...
MANIFEST_IN  = META_DIR / "features_manifest.csv"               # read-only (for skipping)
MANIFEST_OUT = META_DIR / "features_manifest.fullscan.csv"      # write here (safe copy)
JOBS_DIR   = META_DIR / "ptm_jobs"
# FLEURS list used to scope the scan strictly to those files:
FLEURS_CSV = META_DIR / "thirdparty_real_test.fleurs.csv"
# Separate TODO just for FLEURS:
...
def processed_strong_path_for(orig_path: str) -> Path:
    ...
    # mirror full path tail under processed/wav/strong/...
    try:
...
# Map FLEURS originals -> processed STRONG paths
...
for p in fl_paths:
    ...
    if strong_p.exists():
        ...
        # not preprocessed yet; skip (runner is resume-safe)
        pass
...
# Re-scan: mark DONE if npy exists or manifest already has it (from either manifest)
done_manifest = set()
...
with tqdm(total=len(pending), desc=f"{PTM_NAME}:FLEURS:{PROFILE}", unit="file") as pbar:
    for idx, row in pending.iterrows():
        pa, profile, vp = row["path_audio"], row.get("profile", PROFILE), row["vec_path"]
        try:
